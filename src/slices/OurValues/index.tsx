 ...
 * Props for `OurValues`.
 */
export type OurValuesProps = SliceComponentProps<Content.OurValuesSlice>;

/**
 * Component for "OurValues" Slices.
 */
const OurValues: React.FC<OurValuesProps> = ({ slice }) => {
    const { primary } = slice;
    ...
    return (
        ...
        >
            ...
            <div className="container mx-auto px-5 relative z-10">
                ...
                {/* Values Grid */}
                {isFilled.group(primary.value_item) && primary.value_item.length > 0 && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
                        {primary.value_item.map((item, index) => (
                            <div
                                key={index}
                                className="group relative overflow-hidden rounded-xl bg-gradient-to-br from-card/50 to-primary/10 p-8 backdrop-blur-sm transition-all duration-500 hover:scale-105 hover:shadow-lg hover:shadow-primary/20"
                                style={{
                                ...
                                <div className="relative z-10 text-center">
                                    ...
                                    {isFilled.image(item.icon) && (
                                        <div className="mb-6 flex justify-center">
                                            <div className="w-20 h-20 bg-gradient-to-r from-primary to-secondary rounded-full flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform duration-300">
                                                <PrismicNextImage
                                                    field={item.icon}
                                                    className="w-10 h-10"
                                                    alt=""
